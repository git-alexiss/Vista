...
// Build real OAuth URLs using auto-detected BASE_URL
    ...
    if (!verifyCSRF($_POST['csrf']??'')) {
        ...
        if (empty($inputEmail) || empty($inputPass)) {
            ...
            if ($user) {
                $otp = generateOTP();
                setOTP($otp);
                $_SESSION['pending_login'] = buildSessionUser($user);
                $result = sendOTPEmail($user['email'], $user['name'], $otp);
                if (!$result['success']) {
                    error_log('Brevo OTP send failed: ' . $result['error']);
...
<div class="login-container">
  <div class="login-form">
    ...
    <?php if(isset($_SESSION['notification'])): ?>
      <div class="notification success"><?= htmlspecialchars($_SESSION['notification']); unset($_SESSION['notification']); ?></div>
    <?php endif; ?>
    ...
    <?php endif; ?>
    <?php if($locked): ?>
      <div class="notification error">Account locked. Wait <strong><?= remainingLockoutMinutes() ?></strong> min.</div>
    <?php endif; ?>
    ...
    <a href="<?= htmlspecialchars($googleAuthUrl) ?>" class="social-btn google">
      <svg width="20" height="20" viewBox="0 0 48 48">
        ...
        <path fill="#77a6b6" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.97 2.31-8.16 2.31-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
      </svg>
    ...
    <form method="POST">
      ...
      <button type="submit" class="login-btn" <?= $locked?'disabled':'' ?>>Sign In →</button>
    </form>

    <div class="register-link">
      <p>No account yet? <a href="regform.php">Register here</a></p>
    </div>
    <div class="footer"><p>Tourist Satisfaction Prediction System · Province of Rizal</p></div>
  </div>
</div>
</body>
</html>